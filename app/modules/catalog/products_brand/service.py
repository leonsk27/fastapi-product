...
class ProductBrandService:
    ...
    # CREATE
    # ----------------------
    def create_product_brand(self, item_data: ProductBrandCreate, session: Session):
    ...
    # GET ONE
    # ----------------------
    def get_product_brand(self, item_id: int, session: Session):
    ...
    # UPDATE
    # ----------------------
    def update_product_brand(self, item_id: int, item_data: ProductBrandUpdate, session: Session):
        item_db = session.get(ProductBrand, item_id)
        if not item_db:
            raise HTTPException(
                status_code=status.HTTP_404_NOT_FOUND, detail=self.no_task
    ...
    # GET ALL PLANS
    # ----------------------
    def get_product_brands(self, session: Session):
        return session.exec(select(ProductBrand)).all()

    # DELETE
    # ----------------------
    def delete_product_brand(self, item_id: int, session: Session):
        item_db = session.get(ProductBrand, item_id)
        if not item_db:
            raise HTTPException(
                status_code=status.HTTP_404_NOT_FOUND, detail=self.no_task
